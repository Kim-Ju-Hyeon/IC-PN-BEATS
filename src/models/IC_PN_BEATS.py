...
class IC_PN_BEATS(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(len(self.n_pool_kernel_size)):
            if self.pooling_mode == 'max':
                pooling_layer = nn.MaxPool1d(kernel_size=self.n_pool_kernel_size[i],
                                             stride=self.n_stride_size[i], ceil_mode=True)
        ...
        for stack_id in range(self.stack_num):
            self.trend_stacks.append(self.create_stack('generic', stack_id=stack_id, theta_type='trend'))

        for stack_id in range(self.stack_num):
            self.seasonality_stacks.append(self.create_stack('generic', stack_id=stack_id, theta_type='seasonality'))
    ...
    def create_stack(self, stack_type, stack_id, theta_type):
    ...
    def forward(self, inputs, interpretability=False):
        ...
        if interpretability:
            if self.stack_num > 0:
                ...
                outputs['per_trend_forecast'] = np.stack(_per_trend_forecast, axis=0)

                outputs['per_seasonality_backcast'] = np.stack(_per_seasonality_backcast, axis=0)
                outputs['per_seasonality_forecast'] = np.stack(_per_seasonality_forecast, axis=0)

            if self.singular_stack_num > 0:
                outputs['singual_backcast'] = np.stack(_singual_backcast, axis=0)
                outputs['singual_forecast'] = np.stack(_singual_forecast, axis=0)
            else:
                outputs['singual_backcast'] = torch.zeros(1)
                outputs['singual_forecast'] = torch.zeros(1)
