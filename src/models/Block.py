...
class N_HiTS_Block(GNN_Block):
    def __init__(self,
                 ...
                 n_pool_kernel_size=16,
                 n_stride_size=2
                 ):
        assert (pooling_mode in ['max', 'average'])

        super().__init__(inter_correlation_block_type=inter_correlation_block_type,
                         n_theta_hidden=n_theta_hidden,
                         thetas_dim=thetas_dim,
                         backcast_length=int(((backcast_length - n_pool_kernel_size) / n_pool_kernel_size) + 1),
                         forecast_length=forecast_length,
                         inter_correlation_stack_length=inter_correlation_stack_length,
                         update_only_message=update_only_message)
